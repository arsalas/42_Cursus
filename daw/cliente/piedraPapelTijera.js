...
/**
 * Obtencion aleatoria de que opcion va a elegir la maquina
 */
const getOption = () => {
...
const isNameValid = () => {
  ...
  if (name.length <= 3 || !isNaN(name.charAt(0))) return false;
  return true;
};
...
const resultGame = (iPlayer, iMachine) => {
  if (iPlayer === iMachine) drawGame();
    ...
    (iPlayer === 0 && iMachine === posibilidades.length - 1) ||
    iPlayer - 1 === iMachine
  )
    winGame();
  else loseGame();
 ...
 */
const findIndexByOption = (option) =>
  posibilidades.findIndex((opt) => opt === option);

/**
 * Accion que se realiza al pulsar el boton de YA
 */
const handleChoosedAction = () => {
...
// EVENTOS
params.play.addEventListener("click", handlePlayAction);
params.start.addEventListener("click", handleChoosedAction);
params.reset.addEventListener("click", handleResetGame);
